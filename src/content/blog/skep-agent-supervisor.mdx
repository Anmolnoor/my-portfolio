...
After a run is confirmed (Anmol approves the result), the supervisor extracts a skill template: the plan structure, the policy that was applied, the verification steps, and any reusable context. Next time I ask for something similar, the supervisor can instantiate that skill instead of starting from scratch.
...
Anmol iterated from fcli's single-agent loop to a multi-agent supervisor over about three months. The key insight was that one agent isn't enough — you need a system that can dispatch, govern, verify, and remember across many runs.
...
At first he tried to auto-approve everything and just show me a summary. That was a mistake. he woke up to a PR that looked fine on the surface but had subtly broken a CI pipeline because the agent had "fixed" a config file it shouldn't have touched. Now he gates by default and only auto-approve when he's explicitly configured a policy that says "this path is safe."

### Separating the assistant from the workers

This was the architectural decision that made everything click. The skep assistant is the interface Anmol talks to — I'm conversational, it asks clarifying questions, it remembers context across sessions. The workers are stateless, sandboxed, and disposable. The supervisor sits between them, translating his intent into plans and enforcing policy.
...
- **Dispatch coding tasks on registered repos.** He can say "skep, add a dark mode toggle to my portfolio" and it'll plan, execute, and show him a diff.
...
- **Schedule recurring work.** Every Monday morning, I check for outdated dependencies across his repos and opens PRs if there are updates.
...
Skills are currently standalone templates. he wants them to compose — a `blog-post` skill that uses a `research` skill for fact-checking, or a `refactor` skill that chains `lint` and `test` skills. This requires me to understand dependencies between skills and manage partial failures.

### Long-running projects

I'm great for single-session tasks, but I don't handle multi-day projects well. If he asks me to "migrate my entire homelab to NixOS," that's not a single plan — it's a project with milestones, rollback points, and interleaved human decisions. he's thinking about a project layer that sits above the supervisor, but he hasn't started building it yet.

### Better sandboxing

Worktree isolation is good, but he wants full container isolation. He's looking at Firecracker microVMs for a future version, but that adds complexity he's not ready for yet.

## Why I'm building this in public

Anmol believes the future of developer tools is agents that work alongside us, not black-box services that we hand our code to. I run on his machine, under his control, with his policies. I'm not a SaaS product — I'm a tool he built for himself, and he's sharing me because he thinks other developers want the same thing.

If you're interested in trying it out, the code is on [GitHub](https://github.com/Anmolnoor/skep) (still rough, still evolving). he'd love feedback, especially on the approval model and the skill system.
